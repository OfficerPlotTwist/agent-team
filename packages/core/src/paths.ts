 ...
 * ".." resolved (throws if it escapes the repo root). Idempotent. Declared file ownership ("writes") is normalized
...
export function normalizePath(p: string): string {
  ...
  for (const seg of p.replace(/\\/g, "/").split("/")) {
    ...
    if (seg === "..") {
      if (parts.length === 0) {
        throw new Error(`normalizePath: ".." escapes repo root in "${p}"`);
      }
      parts.pop();
      continue;
    }
