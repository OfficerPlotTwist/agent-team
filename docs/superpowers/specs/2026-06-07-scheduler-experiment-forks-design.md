...
3. **Selector: rank + report + auto-promote the winner (tournament).** A
   host-level Selector ranks the staged variants by the metrics and promotes the
   winner to a stable `agentteam/winner-${taskId}` branch — a fast-forward merge
   from the frozen base (trivial: the winner forked from it), non-destructive
   (your working branch + `main` are untouched). ALL variant branches are kept;
   the winner is *additionally* promoted. Per-variant full diffs (`git diff
   base..branch`) are written as artifacts for side-by-side comparison. True
   in-Scheduler continuation (a downstream node consuming the winner mid-run)
   stays deferred — the Selector is a post-run host step, not an in-graph node.
...
packages/host-experiment/
  ...
  src/
    ...
    selector.ts           # rankVariants + selectWinner (pure) — tournament ranking
    report.ts             # renderReport(results, winner) -> { markdown, json } (pure)
    compose.ts            # composeExperiment(opts): Scheduler + AmbientIntegration + per-variant adapterFor
    cli.ts                # agent-team-experiment entry: run -> collect -> select -> promote winner + write diffs
...
completed`). The JSON sibling is for programmatic use. `renderReport` marks the
selected winner row.
...
import { rankVariants } from "./report.js"; // ranking lives in report.ts — one place, no cycle

/** The winner = the top-ranked COMPLETED variant, or null if all failed. */
export function selectWinner(metrics: VariantMetrics[]): VariantMetrics | null;
```

Pure. `report.ts` owns + exports `rankVariants` (completed-before-failed, then
cost asc, then turns asc) and marks the winner row; `selector.ts` reuses it for
`selectWinner`; the CLI imports `selectWinner` to act (promote the winner
branch). The dependency is one-directional (`selector → report`), so no import
cycle. No git here — the Selector decides; the CLI performs the side effect (§5.6).
...
attaches `MetricsCollector`, `run()`, `collect()`. Then the **tournament step**:
`selectWinner(metrics)` → if a winner exists, promote it to
`agentteam/winner-${taskId}` (`git branch -f` at the winner's branch — a
fast-forward from the frozen base, non-destructive). Writes per-variant full
diffs (`git diff ${base}..${branch}`) to `<reportDir>/<variant>.diff` for
side-by-side comparison, then `renderReport()` (markdown + json) and prints the
ranked table, the winner, and the kept branch names.
...
expandTask(task, variants)
   ...
   → selectWinner(metrics)        [top completed by rank]
   → promote winner -> agentteam/winner-${taskId}  (git branch -f; ff from base)
   → write per-variant <variant>.diff artifacts (git diff base..branch)
   → renderReport (ranked md + json, winner marked)
   → all K variant branches kept; winner additionally promoted
```
...
- No **in-Scheduler** continuation — a downstream node consuming the winner
  *mid-run*. The Selector is a post-run host step that promotes the winner to a
  branch; experimenting a node embedded mid-graph (dependents building on the
  winner inside the same `scheduler.run`) stays deferred. (Auto-promote of the
  winner IS in scope — see §2.3 / §5.4b.)
- No LLM-judge scoring (winner chosen by the objective metric rank).
- No auto-merge of the winner onto `main`/an evolving line — promotion lands on a
  dedicated `agentteam/winner-${taskId}` branch only; merging onto a live target
  is a future `--merge-onto` flag.
...
2. The tournament step selects the top-ranked completed variant, promotes it to
   `agentteam/winner-${taskId}` (non-destructive; `main` untouched), and writes a
   `<variant>.diff` artifact per variant for side-by-side comparison. If every
   variant failed, no winner is promoted and the report says so.
3. Zero diff outside `packages/host-experiment` (+ root workspace wiring).
4. Build + tests green per §8 gate (> 220).
5. The isolation test proves no cross-variant memory contamination.
