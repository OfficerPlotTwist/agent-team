...
- A budget-refused node emits `error` and **does no work** (no spend; its file never reaches the integration tip). Since the core robustness pass (2026-05-28, `docs/superpowers/specs/2026-05-28-core-robustness-design.md`) gates completion on a real `done` event, a refused node (error, no `done`) correctly reports in `ScheduleResult.blocked` — not `completed`.
